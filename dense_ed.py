...
- add temporal input in the latent layer
        ...
        if encoding:
            ...
            if last:
                out_convt = nn.ConvTranspose2d(out_features, out_channels,
                                kernel_size=6, stride=2, padding=1, bias=False)
            else:
...
class DenseEDT(nn.Module):
    """
    Surrogate: y = f(x, t)

    Enter time in the latent
    """
    def __init__(self, in_channels, out_channels, blocks, times,
                 growth_rate=16, num_init_features=64, time_channels=20,
                 drop_rate=0, bn_size=8, bottleneck=False):
        """
        Args:
            in_channels (int): number of input channels
            out_channels (int): number of output channels
            blocks: list (of odd size) of integers
            times (list): time instances in training data
            time_channels (int): number of time channels added to the latent space
            growth_rate (int): K
        ...
        self.encoder.add_module('in_conv', nn.Conv2d(in_channels,
                                                     num_init_features,
                                                     kernel_size=7,
                                                     stride=2,
                                                     padding=3,
        ...
        for i, num_layers in enumerate(enc_block_layers):
            block = _DenseBlock(num_layers=num_layers,
                                ...
                                drop_rate=drop_rate,
                                bottleneck=bottleneck)
            self.encoder.add_module('EncBlock%d' % (i + 1), block)
            ...
            trans = _Transition(in_features=num_features,
                                ...
                                encoding=True,
                                drop_rate=drop_rate)
            self.encoder.add_module('Down%d' % (i + 1), trans)
        ...
        # add extra feature in latent
        num_features += self.time_channels
        ...
        for i, num_layers in enumerate(dec_block_layers):
            block = _DenseBlock(num_layers=num_layers,
                                ...
                                drop_rate=drop_rate,
                                bottleneck=bottleneck)
            self.decoder.add_module('DecBlock%d' % (i + 1), block)
            ...
            self.decoder.add_module('Up%d' % (i + 1), trans)
            num_features = num_features // 2

        print("number of parameters: {}\nnumber of layers: {}"
              .format(*self._num_parameters_convlayers()))


    def forward(self, x, times):
        """
        Args:
            x (Variable): (N, iC, iH, iW), a batch of input
            times (Variable): (N, ) 1D Variable representing the time instances
        """
        # N x zC x zH x zW
        x = self.encoder(x)
        # N x 1 x zH x zW
        # print(times)
        # .unsqueeze(-1) is used to add one dimension at the end of current tensor, while unsqueeze(0) add one dimension at the begining of current tensor
        t = times.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(
            x.size(0), self.time_channels, *x.size()[2:])

        y = self.decoder(torch.cat((x, t), 1))
    ...
    def _count_parameters(self):
        ...
        for name, param in self.named_parameters():
            ...
            n_params += param.numel()
            print(
            'num of parameters so far: {}'.format(n_params))
